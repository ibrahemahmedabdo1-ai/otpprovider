...
export default function ClientProfile() {
  ...
  return (
    <div>
      ...
      {tab === 0 && <Overview client={client} onStatusChange={load} />}
      {tab === 1 && <Transactions clientId={id} onShipped={load} />}
      {tab === 2 && <Chat clientId={id} />}
      {tab === 3 && <Diagnose clientId={id} />}
    </div>
  );
}

function Overview({ client, onStatusChange }) {
  ...
  return (
    <div className="card">
      <div className="grid-2">
        ...
        <div><label>API Key</label><p style={{ fontFamily: 'monospace' }}>{client.apiKey}</p></div>
...
function Transactions({ clientId, onShipped }) {
  ...
  return (
    <div>
      <div className="card">
        ...
        <form onSubmit={ship}>
          ...
          <select required value={selectedPackage} onChange={(e) => setSelectedPackage(e.target.value)}>
            ...
            {packages.map((p) => (
              <option key={p.id} value={p.id}>{p.name} ({p.messageCount} رسالة - {p.price} {p.currency})</option>
            ))}
          </select>
      ...
      <div className="card">
        <h3>سجل العمليات</h3>
        <table>
          <thead><tr><th>النوع</th><th>القناة</th><th>عدد الرسائل</th><th>المبلغ</th><th>التاريخ</th></tr></thead>
          <tbody>
            {transactions.map((t) => (
              <tr key={t.id}>
